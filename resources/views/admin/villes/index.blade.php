...
@section('content')
        ...
        <div class="row align-items-center">
            <div class="col-md-8">
                ...
                <ol class="breadcrumb m-0">
                    ...
                    <li class="breadcrumb-item"><a href="#">{{ __('messages.menu.city') }}</a></li>
                </ol>
            </div>
            <div class="col-md-4">
                <div class="float-end d-none d-md-block">
                    @can('Creer ville')
                        <a href="{{ route('admin.villes.create') }}" class="btn btn-primary" type="button">
                            <i class="mdi mdi-plus me-2"></i> {{ __('messages.form.newcity') }}
                        </a>
                    @endcan
                </div>
            </div>
    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="card">
                <div class="card-body">
                    <h4 class="card-title">{{ __('messages.form.cityList') }}</h4>
                    <table id="datatable" class="table table-bordered dt-responsive nowrap"
                        style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>{{ __('messages.form.region') }}</th>
                                <th>{{ __('messages.menu.city') }}</th>
                                <th>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <h6 class="table-avatar">
                                            <a href="#">{{ $ville->title_ville }}</a>
                                        </h6>
                                    </td>
                                    <td>
                                        <div class="form-button-action">
                                            @can('Modifier ville')
                                                <a href="{{ route('admin.villes.edit', $ville->id) }}"><button type="button"
                                                        data-toggle="tooltip" title="" class="btn btn-primary btn-sm"
                                                        data-original-title="{{ __('messages.form.update') }}">
                                                        <i class="fa fa-pen"></i> {{ __('messages.form.update') }}
                                                    </button></a>
                                            @endcan
                                            @can('Supprimer ville')
                                                <button type="button" onclick="deleteData({{ $ville->id }})"
